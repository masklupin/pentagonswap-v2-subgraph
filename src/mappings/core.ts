...
import { updatePairDayData, updateTokenDayData, updatePentagonDayData, updatePairHourData } from './dayUpdates'
...
export function handleTransfer(event: Transfer): void {
  ...
  let factory = PentagonFactory.load(FACTORY_ADDRESS)
  ...
  // case where direct send first on MATIC withdrawls
...
export function handleSync(event: Sync): void {
  ...
  let pentagon = PentagonFactory.load(FACTORY_ADDRESS)

  // reset factory liquidity by subtracting onluy tarcked liquidity
  pentagon.totalLiquidityMATIC = pentagon.totalLiquidityMATIC.minus(pair.trackedReserveMATIC as BigDecimal)
  ...
  // update MATIC price now that reserves could have changed
  let bundle = Bundle.load('1')
  bundle.maticPrice = getMaticPriceInUSD()
  bundle.save()

  token0.derivedMATIC = findMaticPerToken(token0 as Token)
  token1.derivedMATIC = findMaticPerToken(token1 as Token)
  token0.save()
  token1.save()
  ...
  if (bundle.maticPrice.notEqual(ZERO_BD)) {
    trackedLiquidityMATIC = getTrackedLiquidityUSD(pair.reserve0, token0 as Token, pair.reserve1, token1 as Token).div(
      bundle.maticPrice
    )
  } else {
  ...
  pentagon.totalLiquidityMATIC = pentagon.totalLiquidityMATIC.plus(trackedLiquidityMATIC)
  pentagon.totalLiquidityUSD = pentagon.totalLiquidityMATIC.times(bundle.maticPrice)
...
export function handleMint(event: Mint): void {
  ...
  pentagon.txCount = pentagon.txCount.plus(ONE_BI)
...
export function handleSwap(event: Swap): void {
  ...
  let pentagon = PentagonFactory.load(FACTORY_ADDRESS)
  pentagon.totalVolumeUSD = pentagon.totalVolumeUSD.plus(trackedAmountUSD)
  pentagon.totalVolumeMATIC = pentagon.totalVolumeMATIC.plus(trackedAmountMATIC)
  pentagon.untrackedVolumeUSD = pentagon.untrackedVolumeUSD.plus(derivedAmountUSD)
  pentagon.txCount = pentagon.txCount.plus(ONE_BI)
  ...
  let pentagonDayData = updatePentagonDayData(event)
  let token0DayData = updateTokenDayData(token0 as Token, event)
  let token1DayData = updateTokenDayData(token1 as Token, event)

  // swap specific updating
  pentagonDayData.dailyVolumeUSD = pentagonDayData.dailyVolumeUSD.plus(trackedAmountUSD)
  pentagonDayData.dailyVolumeMATIC = pentagonDayData.dailyVolumeMATIC.plus(trackedAmountMATIC)
  pentagonDayData.dailyVolumeUntracked = pentagonDayData.dailyVolumeUntracked.plus(derivedAmountUSD)
  pentagonDayData.save()
  ...
  token0DayData.dailyVolumeMATIC = token0DayData.dailyVolumeMATIC.plus(amount0Total.times(token1.derivedMATIC as BigDecimal))
